...
(defn elide-core-ns [s]
  #?(:cljs (-> s
               ...
               (string/replace "cljs/core/" ""))
     :cljr (string/replace s "clojure.core/" "")))

(defn elide-spec-ns [s]
  #?(:cljs (-> s
               (string/replace "cljs.spec.alpha/" "")
               (string/replace "cljs/spec/alpha" ""))
     :cljr (string/replace s "clojure.spec.alpha/" "")))
...
#?(:cljs
   ...
   :cljr (def format clojure.core/format))
...
(defn escape-replacement [#?(:cljr pattern :cljs _pattern) s]
  #?(:cljr (if (string? pattern)
            s
