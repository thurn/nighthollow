...
(defn ^:private named? [x]
  #?(:cljr (instance? clojure.lang.Named x)
  ...
  (let [{:keys [sym spec failure] :or {sym '<unknown>}} check-result
    ...
    (str
     ...
     (cond
       ...
       #?(:cljr (and failure (= :no-gen failure-reason))
          ;; Workaround for CLJS
          :cljs (and
                 failure
